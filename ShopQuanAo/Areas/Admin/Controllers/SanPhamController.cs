...
namespace ShopQuanAo.Areas.Admin.Controllers
{
	...
	public class SanPhamController : Controller
	{
		...
		{
			// Lấy danh sách sản phẩm (Bao gồm dữ liệu bảng DanhMuc)
			var query = _context.SanPhams.Include(s => s.DanhMuc).AsQueryable();

			// Chức năng tìm kiếm: Lọc theo Tên sản phẩm hoặc Mã sản phẩm
			if (!string.IsNullOrEmpty(searchKeyword))
			{
				searchKeyword = searchKeyword.Trim();

				query = query.Where(s =>
					s.TenSP.Contains(searchKeyword) ||
					s.MaSP.ToString() == searchKeyword
				);
			}

			// Sắp xếp sản phẩm mới nhất lên đầu
			var sanPhams = await query.OrderByDescending(s => s.MaSP).ToListAsync();
